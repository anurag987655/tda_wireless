...
x = gaussian_pulse_shaping(symbols, samples_per_symbol=20)

signal = x/np.std(x)

plt.figure(figsize=(10, 6))
plt.plot(signal.real[:300], label="In-Phase (I)", color="dodgerblue")
plt.plot(signal.imag[:300], label="Quadrature (Q)", color="darkorange")
plt.title("Pulse Shaped QPSK (I & Q components)")
...
for s in SNR: 
    ...
    r_normalized = r / np.sqrt(1.0 + 10**(-s / 10))

    for tau_val in [2, 5, 10, 15, 20]:
        embedded = delay_embedding(r_normalized, tau_val)
        embedded_subsampled = subsample(embedded, max_points =200)
        plot_embedding(embedded_subsampled, tau_val, f"Pulse_shaped_QPSK_AWGN_{s}DB")